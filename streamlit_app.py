...
if st.session_state.show_block_2:
    with st.expander("", expanded = True):
        ...
        if st.button("Загрузить данные"):
            with st.spinner("Загрузка данных... Это может занять до 10 минут"):
                if gender != "Все":
                    ...
                    st.session_state.df = df.drop(columns = ['min_age', 'max_age', 'age_category'])
                if gender == "Все":
                    ...
                    try:
                        ...
                        st.session_state.df = df_all.drop(columns = ['min_age', 'max_age', 'age_category'])
        ...
        if "df" in st.session_state and st.session_state.df is not None:
            ...
            encoding = 'utf-8' if sys.platform != 'win32' else 'windows-1251'
            with col1:
                st.download_button(
                    label = "Скачать CSV",
